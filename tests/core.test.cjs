...
test('cuisine and difficulty intersect without equating a cuisine to one level',()=>{
 for(const c of ['川菜','鲁菜','湘菜','粤菜','苏菜','浙菜','闽菜','徽菜','津菜'])assert.ok(C.filter(R,{cuisine:c}).length,c);
 for(const r of R){assert.ok([1,2,3].includes(r.difficulty),r.name);assert.ok(r.difficultyReason);assert.ok(r.cuisine);}
 const hard=C.filter(R,{difficulty:3});assert.ok(hard.some(r=>r.name==='香蕉厚蛋烧'));assert.ok(!R.some(r=>r.name==='水煮肉片'));
 assert.ok(C.filter(R,{cuisine:'川菜',difficulty:2}).some(r=>r.name==='麻婆豆腐'));
 assert.equal(C.filter(R,{cuisine:'川菜',category:'早餐'}).length,0);
 assert.ok(C.filter(R,{query:'山东菜'}).some(r=>r.name==='油焖大虾'));
 assert.equal(R.find(r=>r.name==='红烧肉').difficulty,1);
 assert.ok(R.find(r=>r.name==='红烧肉').minutes>45,'duration is separate from skill');
});
test('exact catalog, unique IDs/names, full quantified ingredients and steps',()=>{assert.equal(R.length,174);assert.equal(new Set(R.map(r=>r.id)).size,174);assert.equal(new Set(R.map(r=>r.name)).size,174);assert.equal(R.filter(r=>r.category==='家常菜').length,94);assert.equal(R.filter(r=>r.category==='早餐').length,80);for(const r of R){assert.equal(r.servings,2);assert.ok(r.minutes>0);assert.ok(r.steps.length>=3,r.name);assert.ok(r.sources.length);for(const s of r.steps){assert.ok(s.title&&s.heat&&s.text&&s.check);assert.ok(s.seconds>=0);}for(const i of r.ingredients){assert.ok(i.name&&i.unit);assert.ok(Number.isFinite(i.amount)&&i.amount>=0);}assert.ok(r.ingredients.some(i=>i.name==='盐'));assert.ok(r.ingredients.some(i=>i.name==='食用油'));assert.equal(new Set(r.ingredients.map(i=>i.name)).size,r.ingredients.length,r.name);}});
...
test('filters intersect and exclusion never selects forbidden ingredient',()=>{const pool=C.filter(R,{category:'早餐',max:20,noSpicy:true,exclude:'牛奶、鸡蛋',query:''});assert.ok(pool.length);assert.ok(pool.every(r=>r.category==='早餐'&&r.minutes<=20&&!r.spicy&&!r.ingredients.some(i=>/牛奶|鸡蛋/.test(i.name))));assert.equal(C.filter(R,{query:'不存在的食材XYZ'}).length,0);assert.equal(C.filter(R,{category:'家常菜'}).length,94);});
