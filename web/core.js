(function(root){
 ...
 function filter(recipes,f={}){const excluded=(f.exclude||'').split(/[，,、\s]+/).filter(Boolean);return recipes.filter(r=>(!f.category||r.category===f.category)&&(!f.cuisine||r.cuisine===f.cuisine)&&(!f.difficulty||r.difficulty===Number(f.difficulty))&&(!f.group||r.group===f.group)&&(!f.newOnly||r.edition==='0.2.0')&&(!f.noNoodles||r.group!=='面条粉类')&&(!f.max||r.minutes<=Number(f.max))&&(!f.noSpicy||!r.spicy)&&(!f.query||(r.name+' '+({鲁菜:'山东菜',川菜:'四川菜 重庆菜',湘菜:'湖南菜',粤菜:'广东菜',苏菜:'江苏菜 淮扬菜',浙菜:'浙江菜 杭州菜',闽菜:'福建菜',徽菜:'安徽菜 徽州菜',津菜:'天津菜',东北风味:'东北菜'}[r.cuisine]||'')+' '+(r.cuisine||'')+' '+(r.aliases||[]).join(' ')+' '+r.group+' '+r.ingredients.map(i=>i.name).join(' ')).includes(f.query.trim()))&&excluded.every(x=>!(r.name+' '+r.ingredients.map(i=>i.name).join(' ')+' '+r.allergens.join(' ')).includes(x)));}
